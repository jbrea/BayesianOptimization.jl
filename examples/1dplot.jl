...
Random.seed!(134)
function f(x, noisevariance = 1)
    0.1 * sum((x .- 2) .^ 2) + cos(sum(π / 2 * x)) + noisevariance * randn()
end
model = ElasticGPE(1, mean = MeanConst(0.0),
                   kernel = SEArd([0.0], 5.0), logNoise = 0.0)
modeloptimizer = MAPGPOptimizer(every = 50, noisebounds = [-2.0, 3],
                                kernbounds = [[-1, 0], [4, 10]], maxeval = 40)
opt = BOpt(f, model, ExpectedImprovement(),
           modeloptimizer, [-5.0], [5.0],
...
@pgf GroupPlot({group_style = {group_size = "1 by 2", vertical_sep = "4mm"},
               ...
               Plot({no_marks, "red!20", name_path = "A", forget_plot},
                    Coordinates(xs, -ms .+ sig)),
               Plot({no_marks, "red!20", name_path = "B", forget_plot},
                    Coordinates(xs, -ms .- sig)),
               ...
               {height = "3cm", ytick = [0, 0.05, 0.1], yticklabels = [0, "0.05", "0.1"]},
